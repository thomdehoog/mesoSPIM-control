...
Background: Live3DPyramidWriter flushes one chunk-deep, full-frame slab per
write, so every shard must be exactly one slab deep. A deeper shard takes
several slab writes to fill; each one makes Zarr read the whole shard file
back, merge, and rewrite it, and because slabs are flushed concurrently
those rewrites race and silently lose planes (reproduced 5 runs out of 5
before the fix).

These tests pin four things:

* the shard z-depth is clamped to the chunk z-depth, whatever the config says;
* data written under the previously corrupting config round-trips intact;
* the writer never reads shard data back while writing (read-back is the
  signature of the slow rewrite path);
* close() writes out frames still waiting in the ingest queue instead of
  dropping them.
