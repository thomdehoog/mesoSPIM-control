...
from mesoSPIM.src.plugins.support_files.ImageWriters.OmeZarrWriterMP.omezarr_writer import (
    BloscCodec, BloscShuffle, ChunkScheme, FlushPad, Live3DPyramidWriter, PyramidSpec, TCZYX,
    compute_xy_only_levels, plan_levels,
)
...
def run(folder: Path, planes: int, yx: int, chunk_z: int, shards, label: str) -> dict:
    ...
    writer = Live3DPyramidWriter(
        spec, voxel_size=voxel, path=str(path), max_workers=2, max_inflight_chunks=8,
        ...
        tczyx=TCZYX(t=0, c=0, n_channels=1, channel_labels=("488",)),
    )
    for frame in frames:
