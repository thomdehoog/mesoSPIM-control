...
from mesoSPIM.src.plugins.support_files.ImageWriters.OmeZarrWriterMP.omezarr_writer import (
    PyramidSpec, ChunkScheme, TCZYX,
    plan_levels, compute_xy_only_levels, FlushPad,
...
class OMEZarrWriterMPTCZYX(OMEZarrWriterMP):
    ...
    the frames travel through the same shared-memory ring buffer to the same writer process,
    which runs the same live multiscale pipeline (Live3DPyramidWriter).
    ...
    def open(self, req: WriteRequest) -> None:
        ...
        self._writer_proc = ctx.Process(
            ...
            args=(
                ...
                self._work_q,
                self._free_q,
                None,  # write_cache: not supported for a store that grows
            ),
            daemon=True,
        )
