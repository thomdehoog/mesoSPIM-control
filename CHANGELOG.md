...
- 💎 **Data viewer** (`View → Open Data Viewer`): a separate window that shows the acquisition being written, tile by tile and time point by time point, within a second of each landing. It follows the newest acquisition in the folder the acquisition list saves into; a dropdown in its panel switches to earlier ones of the session. 2D/3D, per-channel window with histogram and colour, depth and time sliders, projection and detail controls in 3D. It reads the `MP_OME_Zarr_TCZYX_Writer` layout and comes from the separate `mesospim_view` package (`pip install "git+https://github.com/thomdehoog/ZMART-viewer@mesospim-view"` plus `PyQtWebEngine`); without it the menu entry says so. See :doc:`data_viewer` in the docs for installing and testing it.
...
- Fixed unzeroing galvo amplitude getting stuck during interactive optimization.
- Support for multi-part file extensions (e.g., `.ome.zarr`) and dots in filenames (e.g., `Mag7.5`).
- Proper handling of special characters in metadata (% signs removal, underscore replacement).
